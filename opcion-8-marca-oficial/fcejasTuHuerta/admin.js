...
// ---------- Descargar archivo para publicar ----------
// Regenera products-data.js completo: toma el archivo real del repo (para no perder
// funciones ni comentarios), reemplaza solo el bloque marcado con @DEFAULT_PRODUCTS
// y sella la fecha en DEFAULT_PRODUCTS_UPDATED_AT. Esa fecha es la que decide qué
// versión es la más nueva cuando se sube a GitHub (ver products-data.js).
async function buildDataFileText(products, updatedAt) {
  const res = await fetch(new URL('../products-data.js', location.href), { cache: 'no-store' });
  if (!res.ok) throw new Error('No se pudo leer products-data.js (' + res.status + ')');
  let text = await res.text();

  const START = '/* @DEFAULT_PRODUCTS:start */';
  const END = '/* @DEFAULT_PRODUCTS:end */';
  const s = text.indexOf(START);
  const e = text.indexOf(END);
  if (s === -1 || e === -1) throw new Error('No se encontraron los marcadores @DEFAULT_PRODUCTS.');

  const block = `${START}\nconst DEFAULT_PRODUCTS = ${JSON.stringify(products, null, 2)};\n${END}`;
  text = text.slice(0, s) + block + text.slice(e + END.length);
  text = text.replace(
    /const DEFAULT_PRODUCTS_UPDATED_AT = '[^']*';/,
    `const DEFAULT_PRODUCTS_UPDATED_AT = '${updatedAt}';`
  );
  return text;
}
...
async function handleDownload() {
  ...
  if (products.some(p => !p.name)) {
    setStatus('Hay productos sin nombre. Completalos antes de descargar.', true);
    return;
  }
  const updatedAt = new Date().toISOString();
  let text;
  try {
    text = await buildDataFileText(products, updatedAt);
  } catch (err) {
    setStatus('No se pudo generar el archivo. Abrí el panel desde el sitio publicado o el servidor local (http/https), no como archivo suelto. ' + err.message, true);
    return;
  }
  // Dejamos este navegador sincronizado con lo que se va a publicar.
  saveProducts(products, updatedAt);
  downloadFile('products-data.js', text);
  renderTable();
  setStatus('Archivo products-data.js descargado (versión ' + updatedAt + '). Subilo a GitHub reemplazando el actual para publicar los cambios.');
}
...
$('exportBtn').addEventListener('click', handleDownload);
